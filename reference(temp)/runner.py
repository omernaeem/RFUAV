...
transform = transforms.Compose([
    transforms.Resize((224, 224)),  # Swintransformer 需要输入224x224大小的图像
...
Swintransformer = models.swin_t(pretrained=True)

# 修改最后的全连接层，设置为你的类别数（假设有10个类别）
num_classes = 10
Swintransformer.fc = nn.Linear(Swintransformer.head.in_features, num_classes)

# 定义损失函数和优化器
criterion = nn.CrossEntropyLoss()  # 用于分类任务
optimizer = optim.Adam(Swintransformer.parameters(), lr=0.001)  # 使用Adam优化器，学习率可以调整
...
for epoch in range(num_epochs):
    Swintransformer.train()  # 将模型设置为训练模式
    ...
    for images, labels in train_loader:
        ...
        # 前向传播
        outputs = Swintransformer(images)
    ...
    with torch.no_grad():
        for val_images, val_labels in val_loader:
            ...
            val_outputs = Swintransformer(val_images)
